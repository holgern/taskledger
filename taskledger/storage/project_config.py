...
def merge_project_config(
    ...
    worker_pipeline = _parse_worker_pipeline(
...
def _validate_project_config_overrides(data: dict[str, object], path: Path) -> None:
    ...
    _validate_worker_pipeline(data.get("worker_pipeline"), path)
    _validate_sync_config(data.get("sync"), path)


def _validate_worker_pipeline(raw: object, path: Path) -> None:
    if raw is None:
        return
    ...
    for index, step in enumerate(steps, start=1):
        step_id = _validate_worker_step(step, path, index=index)
...
def _validate_worker_step(data: dict[str, object], path: Path, *, index: int) -> str:
    _validate_worker_step_unknown_keys(data, path, index=index)
    step_id = _validate_worker_step_id(data, path, index=index)
    _validate_worker_step_optional_text_fields(data, path, index=index)
    _validate_worker_step_required_choice(
        data,
    ...
    _validate_worker_step_optional_choice(
        data,
    ...
    _validate_worker_step_string_list(data, "required_output", path, index=index)
    _validate_worker_step_string_list(data, "must_not", path, index=index)
    return step_id


def _validate_worker_step_unknown_keys(
...
def _validate_worker_step_optional_text_fields(
    data: dict[str, object],
    path: Path,
    *,
    index: int,
...
def _validate_worker_step_required_choice(
    data: dict[str, object],
    *,
    field_name: str,
...
def _validate_worker_step_optional_choice(
    data: dict[str, object],
    *,
    field_name: str,
    valid_values: frozenset[str],
...
def _validate_worker_step_string_list(
    data: dict[str, object],
    field_name: str,
    path: Path,
    *,
...
def _parse_worker_pipeline(
    ...
    steps = tuple(
        _parse_worker_step(item) for item in step_items if isinstance(item, dict)
    )
...
def _parse_worker_step(raw: dict[str, object]) -> WorkerStepConfig:
    step_id = str(raw["id"])
    base_context = str(raw["base_context"])
    actor_role = (
        str(raw["actor_role"])
        if isinstance(raw.get("actor_role"), str)
        else _default_worker_actor_role(base_context)
    )
    label = (
        str(raw["label"])
        if isinstance(raw.get("label"), str)
        else _default_worker_step_label(step_id)
    )
    required_output = _worker_string_tuple(raw.get("required_output"))
    must_not = _worker_string_tuple(raw.get("must_not"))
...
def _default_worker_actor_role(base_context: str) -> str:
...
def _default_worker_step_label(step_id: str) -> str:
    return step_id.replace("-", " ").replace("_", " ").title()


def _worker_string_tuple(raw: object) -> tuple[str, ...]:
