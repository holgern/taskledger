...
def test_import_single_task_renumber_remaps_changelog_entry_task_id(
    tmp_path: Path,
) -> None:
    source_root = tmp_path / "source"
    dest_root = tmp_path / "dest"
    ...
    assert (
        runner.invoke(
            ...
            [
                ...
                "added",
                "--summary",
                "Added changelog entry remap coverage for exchange imports",
            ],
        ).exit_code
        == 0
    )
    ...
    entries = cast(list[dict[str, Any]], entry_list["result"]["entries"])
    assert len(entries) == 1
    assert entries[0]["task_id"] == "task-0002"
