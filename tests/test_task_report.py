...
class TestCLIReport:
    ...
    def test_report_html_output_writes_file(self, tmp_path: Path) -> None:
        ws = init_workspace(tmp_path)
        task_id = create_done_task(ws, allow_lint_errors=True)
        output_path = tmp_path / "task-report.html"
        exit_code, stdout, _ = _invoke(
            ["report", "html", task_id, "--output", str(output_path)],
            cwd=ws,
        )
    ...
    def test_report_html_active_output_writes_file(self, tmp_path: Path) -> None:
        ws = init_workspace(tmp_path)
        task_id = create_done_task(ws, allow_lint_errors=True)
        activate_task(ws, task_id, reason="test active html")
        output_path = tmp_path / "active-task.html"
        exit_code, _, _ = _invoke(
            ["report", "html", "--active", "--output", str(output_path)],
            cwd=ws,
        )
        assert exit_code == 0
        assert output_path.exists()
    ...
    def test_report_html_requires_target_or_active(self, tmp_path: Path) -> None:
        ws = init_workspace(tmp_path)
        create_done_task(ws, allow_lint_errors=True)
        exit_code, stdout, _ = _invoke(
            ["--json", "report", "html", "--output", str(tmp_path / "x.html")],
            cwd=ws,
        )
        assert exit_code == 2
        ...
        assert "TASK_REF or --active is required." in payload["error"]["message"]

    def test_report_site_writes_index_and_tasks(self, tmp_path: Path) -> None:
        ws = init_workspace(tmp_path)
        create_done_task(ws, allow_lint_errors=True)
        output_dir = tmp_path / ".taskledger-report"
        exit_code, _, _ = _invoke(
            ["report", "site", "--output", str(output_dir)],
            cwd=ws,
        )
        assert exit_code == 0
        assert (output_dir / "index.html").exists()
        assert list((output_dir / "tasks").glob("task-*.html"))

    def test_report_html_json_payload_reports_written_file(
        self,
        tmp_path: Path,
    ) -> None:
        ws = init_workspace(tmp_path)
        ...
        assert result["kind"] == "html_task_report_written"
